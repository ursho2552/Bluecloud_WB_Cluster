...
the user, however.")
  ...
  text(x = 0, y = 0.95, cex = 0.7, font = 1, adj = c(0,1), "The following parameters are related to the biological sample selection:")
  text(x = 0, y = 0.9, cex = 0.7, font = 1, adj = c(0,1), paste("     - Minimum number of sample per species:", CALL$SAMPLE_SELECT$MIN_SAMPLE))
  text(x = 0, y = 0.85, cex = 0.7, font = 1, adj = c(0,1), paste("     - Depth range of the samples:", CALL$SAMPLE_SELECT$MIN_DEPTH, "to", CALL$SAMPLE_SELECT$MAX_DEPTH, "m"))
  text(x = 0, y = 0.8, cex = 0.7, font = 1, adj = c(0,1), paste("     - Time range of the samples:", CALL$SAMPLE_SELECT$START_YEAR, "to", CALL$SAMPLE_SELECT$STOP_YEAR))
  text(x = 0, y = 0.7, cex = 0.7, font = 1, adj = c(0,1), "The following parameters are related to the type of data used:")
  text(x = 0, y = 0.65, cex = 0.7, font = 1, adj = c(0,1), paste("     - Type of raw data query from the data access service:", CALL$DATA_SOURCE))
  text(x = 0, y = 0.6, cex = 0.7, font = 1, adj = c(0,1), paste("     - Type of raw data considered in the analysis (after eventual transformation of the raw data):", CALL$DATA_TYPE))
  text(x = 0, y = 0.5, cex = 0.7, font = 1, adj = c(0,1), "The following parameters are related to the algorithm training:")
  text(x = 0, y = 0.45, cex = 0.7, font = 1, adj = c(0,1), paste("     - Number of splits for the cross-validation folds:", CALL$NFOLD))
  text(x = 0, y = 0.4, cex = 0.7, font = 1, adj = c(0,1), paste("     - Type of cross-validation performed:", CALL$FOLD_METHOD))
